  ...
  return {get: (id, callback) => {
    global.distribution.local.groups.get(context.gid, (e, v) => {
      ...
      if (e !== null) {
        ...
        if (id === null) {
          ...
          global.distribution.local.comm.send([toSend], remote, callback);
        }
      }
    });
  }, put: (obj, id, callback) => {
    ...
    global.distribution.local.groups.get(context.gid, (e, v) => {
      ...
      if (e !== null) {
        ...
        const nid = context.hash(util.id.getID(id), Array.from(nidMap.keys()));
        remote = {node: nidMap.get(nid),
          service: 'mem', method: 'del'};
        global.distribution.local.comm.send([{key: id, gid: context.gid}],
            remote, callback);
      }
    });
    ...
    global.distribution[context.gid].mem.get(null, (e, v) => {
      ...
      global.distribution.local.groups.get(context.gid, (e, v) => {
        ...
        for (let k of relocate) {
          let remote = {node: k.old, service: 'mem', method: 'get'};
          global.distribution.local.comm.send([{key: k.id,
            gid: context.gid}], remote, (e, v) => {
            const obj = v;
            remote = {node: k.old, service: 'mem', method: 'del'};
            global.distribution.local.comm.send([{key: k.id,
              gid: context.gid}], remote, (e, v) => {
            });
            remote = {node: k.new, service: 'mem', method: 'put'};
